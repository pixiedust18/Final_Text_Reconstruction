...
from test_single import inpaint
...
def main():
    ...
    higher_Color_val = (1, 1, 1)	#Obere RGB Farbgrenze

    mask = np.zeros(img.shape[:2], dtype="uint8")
    mask = cv2.inRange(img, lower_Color_val, higher_Color_val) 

    #cv2_imshow(mask)
    cv2.imwrite('mask.png', mask)
    inpaint(image = img_path, im_shape = (img.shape[1], img.shape[0]), mask = 'mask.png', output='results/inpainted_im.png', checkpoint_path='/content/gdrive/MyDrive/BE Project/hole_benchmark')    
    inp_im = cv2.imread('results/inpainted_im.png')
    inp_im = cv2.resize(inp_im, (img.shape[1], img.shape[0]))
    cv2.imwrite('results/inpainted_im.png', inp_im)
